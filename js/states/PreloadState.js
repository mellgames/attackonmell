...
AttackOnBall.PreloadState.prototype.preload = function () {
  ...
  for(var i=0; i<=10; i++) {
    ...
    game.load.audio(SOUNDS.RUN, 'assets/sound/run.mp3');
    game.load.audio(SOUNDS.DEATH, 'assets/sound/death.mp3');
    game.load.audio(SOUNDS.COLLECT, 'assets/sound/collect.mp3');
    game.load.audio(SOUNDS.BACKGROUND, 'assets/sound/background.ogg');

  var style = { font: "bold 32px Arial", fill: "#fff", boundsAlignH: "center", boundsAlignV: "middle" };
  var text = game.add.text(0, 0, "Загрузка(0%)", style);
  text.setShadow(3, 3, 'rgba(0,0,0,0.5)', 2);
  text.setTextBounds(0, 0, WIDTH, HEIGHT);

  game.load.onFileComplete.add(function(process) {
    text.text = "Загрузка(" + process + "%)";
  });
